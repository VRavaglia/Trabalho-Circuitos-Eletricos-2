...
int resolversistema(MainWindow &main_window)
{
  int i,j,l, a;
  double t, p;
  ...
  for (i=1; i<=nv; i++) {
    ...
    if (fabs(t)<TOLG) {
      main_window.i_erro("Sistema singular\n");
      return 1;
    }
...
int numero(char *nome, MainWindow &main_window)
{
  int i,achou;
  ...
  if (!achou) {
    if (nv==MAX_NOS) {
      main_window.i_erro("O programa so aceita ate" + QString(nv) +"\n");
      exit(1);
    }
    nv++;
...
int calculo(int argc, char* argv[], MainWindow &main_window)
{
  ...
  /*clrscr();*/system("cls");
  main_window.i_printf("Programa demonstrativo de analise nodal modificada\n");
  main_window.i_printf("Por Antonio Carlos M. de Queiroz - acmq@coe.ufrj.br\n");
  main_window.i_printf("Versao %s\n",versao);
 denovo:
  /* Leitura do netlist */
  ...
  if (argc>1) {
    ...
    main_window.i_printf("Nome do arquivo com o netlist (ex: mna.net): ");
    scanf("%50s",nomearquivo);
  }
  arquivo=fopen(nomearquivo,"r");
  if (arquivo==0) {
    main_window.i_printf("Arquivo %s inexistente.\n",nomearquivo);
    argc=1;
    goto denovo;
  }
  main_window.i_printf("Lendo netlist:\n");
  fgets(txt,MAX_LINHA,arquivo);
  main_window.i_printf("Titulo: %s",txt);
  while (fgets(txt,MAX_LINHA,arquivo)) {
    ne++; /* Nao usa o netlist[0] */
    if (ne>MAX_ELEM) {
      main_window.i_printf("O programa so aceita ate %d elementos.\n",MAX_ELEM);
      exit(1);
    }
    txt[0]=toupper(txt[0]);
    tipo=txt[0];
    ...
    if (tipo=='R' || tipo=='I' || tipo=='V') {
      ...
      main_window.i_printf("%s %s %s %g\n",netlist[ne].nome,na,nb,netlist[ne].valor);
      netlist[ne].a=numero(na, main_window);
      netlist[ne].b=numero(nb, main_window);
    }
    else if (tipo=='S'){
        sscanf(p, "%10s%10s%lg%lg%lg%lg",na,nb,&netlist[ne].valor,
                &netlist[ne].freqOPer,&netlist[ne].phaseOtin,&netlist[ne].preOoffOminVC);
        main_window.i_printf("%s %s %s %g %g %g\n", netlist[ne].nome,na,nb,netlist[ne].valor,
        ...
        sscanf(p, "%10s%10s%lg%lg%lg%lg%lg%lg%lg",na,nb,&netlist[ne].valor,
                ...
                &netlist[ne].dtSteady);
        main_window.i_printf("%s %s %s %g %g %g %g %g %g %g\n", netlist[ne].nome,na,nb,
      ...
      main_window.i_printf("%s %s %s %s %s %g\n",netlist[ne].nome,na,nb,nc,nd,netlist[ne].valor);
      netlist[ne].a=numero(na, main_window);
      netlist[ne].b=numero(nb, main_window);
      netlist[ne].c=numero(nc, main_window);
      netlist[ne].d=numero(nd, main_window);
    }
      ...
      main_window.i_printf("Comentario: %s",txt);
      ne--;
    }
    else {
      main_window.i_erro("Elemento desconhecido: \n"+ QString(txt));
      exit(1);
    }
  }
  fclose(arquivo);
  ...
  for (i=1; i<=ne; i++) {
    ...
    if (tipo=='V' || tipo=='E' || tipo=='F' || tipo=='O' || tipo=='L' ||
      ...
      nv++;
      if (nv>MAX_NOS) {
        main_window.i_printf("As correntes extra excederam o numero de variaveis permitido (%d)\n",MAX_NOS);
        exit(1);
      }
    ...
    itsCur:
  /* Lista tudo */
  main_window.i_printf("Variaveis internas: \n");
  for (i=0; i<=nv; i++)
  main_window.i_printf("%d -> %s\n",i,lista[i]);
  main_window.i_printf("Netlist interno final\n");
  for (i=1; i<=ne; i++) {
    tipo=netlist[i].nome[0];
    if (tipo=='R' || tipo=='I' || tipo=='V') {
      main_window.i_printf("%s %d %d %g\n",netlist[i].nome,netlist[i].a,netlist[i].b,netlist[i].valor);
    }
    else if (tipo=='C' || tipo=='L')
        main_window.i_printf("%s %d %d %g %g\n",netlist[ne].nome,netlist[i].a,netlist[i].b,
                netlist[i].valor,netlist[i].preOoffOminVC);
    else if (tipo=='S' || tipo=='D')
        main_window.i_printf("%s %d %d %g %g %g %g %g %g\n",netlist[i].nome,netlist[i].a,
                netlist[i].b,netlist[i].valor,netlist[i].freqOPer,
                netlist[i].phaseOtin,netlist[i].dTsub,netlist[i].dTdes,
                netlist[i].dtSteady);
    else if (tipo=='G' || tipo=='E' || tipo=='F' || tipo=='H') {
      main_window.i_printf("%s %d %d %d %d %g\n",netlist[i].nome,netlist[i].a,netlist[i].b,netlist[i].c,netlist[i].d,netlist[i].valor);
    }
    else if (tipo=='O') {
      main_window.i_printf("%s %d %d %d %d\n",netlist[i].nome,netlist[i].a,netlist[i].b,netlist[i].c,netlist[i].d);
    }
    if (tipo=='V' || tipo=='E' || tipo=='F' || tipo=='O' || tipo=='L' ||
            tipo=='S' || tipo=='D'){
        if(netlist[i].nome[1]=='I') goto buildSys;
      main_window.i_printf("Corrente jx: %d\n",netlist[i].x);
    }
    else if (tipo=='H')
      main_window.i_printf("Correntes jx e jy: %d, %d\n",netlist[i].x,netlist[i].y);
  }
  /* Monta o sistema nodal modificado */
  main_window.i_printf("O circuito tem %d nos, %d variaveis e %d elementos\n",nn,nv,ne);
  ...
  for (i=1; i<=ne; i++) {
    ...
    if (tipo=='R' || tipo=='C') {
      ...
      Yn[netlist[i].x][netlist[i].b]+=1;
        ...
        if (i != (*curD).elNE){
            main_window.i_printf("Um error interno ocorreu\n");
            exit(1);
        }
        g=0;
    ...
    main_window.i_printf("Sistema apos a estampa de %s\n",netlist[i].nome);
    for (k=1; k<=nv; k++) {
      for (j=1; j<=nv+1; j++)
        if (Yn[k][j]!=0) main_window.i_printf("%+3.1f ",Yn[k][j]);
        else main_window.i_printf(" ... ");
      main_window.i_printf("\n");
    }
    main_window.i_getch();
#endif
  }
  /* Resolve o sistema */
  if (resolversistema(main_window)) {
    main_window.i_printf("Fim da simulação");
    exit(1);
  }
#ifdef DEBUG
  /* Opcional: Mostra o sistema resolvido */
  main_window.i_printf("Sistema resolvido:\n");
  for (i=1; i<=nv; i++) {
      for (j=1; j<=nv+1; j++)
        if (Yn[i][j]!=0) main_window.i_printf("%+3.1f ",Yn[i][j]);
        else main_window.i_printf(" ... ");
      main_window.i_printf("\n");
    }
  main_window.i_getch();
